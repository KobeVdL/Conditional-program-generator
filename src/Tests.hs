...
shrinkToFile:: Int ->Int -> Program -> (Predicate -> Bool) ->IO()

shrinkToFile loopTimes times program propCheck  = do
    results <- shrinkingLoop loopTimes times program propCheck 
    let csv = doubleListToCsv (transpose results)
    writeFile "Shrinking/shrinkingStep3Version2.csv" csv


-- is going to turn the double list into a csv, each array becomes an array in the result
doubleListToCsv:: Show a => [[a]] -> String

doubleListToCsv  [] = "\n"

doubleListToCsv (x:xs) =  singleListToCsv x ++ "\n" ++ doubleListToCsv xs

-- is going to turn the single list into a csv, each array becomes an array in the result
singleListToCsv:: Show a => [a] -> String

singleListToCsv [] = ""

singleListToCsv [x] = show x
    
singleListToCsv (x:xs) = show x ++ ";" ++ singleListToCsv xs  
    
  
    

-- calls the shrinking reduce test given amount of times each time increasing the size with 10
shrinkingLoop:: Int -> Int  -> Program -> (Predicate -> Bool) -> IO [[Int]]

shrinkingLoop 0 _ _ _ = return []

shrinkingLoop loopTimes times program propCheck = do
        other <- shrinkingLoop (loopTimes -1) times program propCheck
        results <- shrinkingReduceTest times (loopTimes*100) program propCheck
        return (results:other)

-- calculates the difference between the generated shrinked term and the minimal term n times  
shrinkingReduceTest :: Int ->Int -> Program -> (Predicate -> Bool) -> IO [Int]

shrinkingReduceTest 0 size program propCheck = return []

shrinkingReduceTest times size program propCheck = do
    predi <- topDownPropertyChecking size maxTries propCheck program
    case predi of
        Just pred -> do
            let shrink = shrinkAlgorithm pred propCheck precond program
            let diff = Set.toList (Set.map (shrinkingReduceDifference program propCheck) shrink) 
            listDiff <- shrinkingReduceTest (times-1) size program propCheck
            return (diff ++ listDiff) 
        Nothing -> shrinkingReduceTest (times-1) size program propCheck
    where
        maxTries= 10000

-- compares the reduced Term with the minimal Term and returns the difference between the two
shrinkingReduceDifference:: Program -> (Predicate -> Bool) -> Predicate ->  Int

shrinkingReduceDifference program propCheck reducedPred =  
    case minimalTerm of
        Just x -> differenceSize reducedPred x
        Nothing -> -1
    where
    minimalTerm = stepMinimalTerm program propCheck


-- Calculates the difference between the sizes of two terms
differenceSize :: Predicate -> Predicate -> Int

differenceSize term1 term2 = abs(sizeOf term1 - sizeOf term2)


-- minimalTermForEachStep
stepMinimalTerm::Program -> (Predicate -> Bool) -> Maybe Predicate

stepMinimalTerm program propCheck = fastBottomUpProperty program propCheck 4 Map.empty Map.empty


{-
step1MinimalTerm = MkPredicate MkTerm "leq" 2 [MkTerm "Zero" 0 [],MkTerm "Zero" 0 []]

step2MinimalTerm::Term

step2MinimalTerm = MkTerm "leq" 2 [MkTerm "Zero" 0 [],MkTerm "Zero" 0 []]

step3MinimalTerm::Term

step3MinimalTerm = MkTerm "succ" 1 [MkTerm "Zero" 0 []]

step4MinimalTerm::Term

step4MinimalTerm = MkTerm ":" 2 [MkTerm "succ" 1 [MkTerm "Zero" 0 []],MkTerm "ifThenElse" 3 [MkTerm "True" 0 [],MkTerm "[]" 0 [],MkTerm "[]" 0 []]]

step5MinimalTerm::Term

step5MinimalTerm = MkTerm ":" 2 [MkTerm "succ" 1 [MkTerm "Zero" 0 []],MkTerm "ifThenElse" 3 [MkTerm "True" 0 [],MkTerm "[]" 0 [],MkTerm "[]" 0 []]]
-}
