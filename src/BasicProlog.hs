...
instance Bindable Term where
    ...
    sizeOf term = sizeOf (values term)+ 1
...
instance Bindable Predicate where    
    ...
    sizeOf (MkPredicate _ 0 []) = 0

    sizeOf pred = sizeOf (valuesOfPred pred)+ 1
...
instance Bindable a => Bindable [a] where
    ...
    sizeOf list = (foldr (\x acc -> sizeOf x + acc) 0 list )
